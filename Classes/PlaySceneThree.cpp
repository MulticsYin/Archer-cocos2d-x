...
Animation* PlaySceneThree::createAnimation(std::string prefixName, int framesNum, float delay)
{
...
void PlaySceneThree::onEnter()
{
	...
	contactListener->onContactBegin = CC_CALLBACK_1(PlaySceneThree::onContactBegin, this);
	contactListener->onContactPostSolve = CC_CALLBACK_2(PlaySceneThree::onContactPostSolve, this);
	contactListener->onContactPreSolve = CC_CALLBACK_2(PlaySceneThree::onContactPreSolve, this);
	contactListener->onContactSeparate = CC_CALLBACK_1(PlaySceneThree::onContactSeparate, this);
	...
	auto pauseMenuItem = MenuItemSprite::create(pauseSprite, pauseSprite,
		CC_CALLBACK_1(PlaySceneThree::menuPauseCallback, this));
	...
	//加入测试用的关卡号
	auto star = Sprite::create("scene3.png");	//静态的
	star->setAnchorPoint(Point(0, 0));
	star->setPosition(visibleSize.width * 9 / 10, visibleSize.height * 9 / 10);
	this->addChild(star, 200);

	//Update每帧执行一次（1/60秒）
	schedule(schedule_selector(PlaySceneThree::Update));
}

bool PlaySceneThree::onTouchBegan(Touch* touches, Event* event)
{
	log("touchBeganNNNNN执行了---------------------------------------------");
...
void PlaySceneThree::onTouchMoved(Touch* touches, Event* event)
{
	log("touchMMMMMMMMMOOVVEDDD执行了---------------------------------------------");
	// 获取当前触摸的目标
	Touch* touch = static_cast<Touch*>(touches);
...
void PlaySceneThree::onTouchEnded(Touch* touches, Event* event)
{
	Touch* touch = static_cast<Touch*>(touches);
	Point locationInNodelocal = touch->getLocation();
	ontouch = 0;
...
bool PlaySceneThree::onContactBegin(PhysicsContact &contact) {
	...
	{
		log("GameOVERisTouchedQQQQQQQQQQQQQQQQQQQQ");
...
bool PlaySceneThree::onContactPreSolve(PhysicsContact& contact, PhysicsContactPreSolve& solve) {
	return true;
}

void PlaySceneThree::onContactPostSolve(PhysicsContact& contact, const PhysicsContactPostSolve& solve) {

}

void PlaySceneThree::onContactSeparate(PhysicsContact& contact) {

}

// 计时器更新函数，每帧一次
void PlaySceneThree::Update(float time) {
	log("UPDATE             !!!!!!!!!!!!");
...
void PlaySceneThree::onExit() {
	Layer::onExit();
	//停止调用 shootBullet函数.
	this->unschedule(schedule_selector(PlaySceneThree::Update));
	//注销事件监听器.
...
void PlaySceneThree::onEnterTransitionDidFinish()
{
...
void PlaySceneThree::isGameOver()
{
	//无法返回游戏界面
		//选择重来
		log("GameOver");
...
void PlaySceneThree::menuPauseCallback(Ref* pSender) {	//暂停按钮
	...
	auto backMenuItem = MenuItemSprite::create(backNormal, backSelected,
		CC_CALLBACK_1(PlaySceneThree::menuBackCallback, this));
	...
	auto resumeMenuItem = MenuItemSprite::create(resumeNormal, resumeSelected,
		CC_CALLBACK_1(PlaySceneThree::menuResumeCallback, this));
...
void PlaySceneThree::menuBackCallback(Ref* pSender)	//回到主界面
{
...
void PlaySceneThree::menuResumeCallback(Ref* pSender)		//回到游戏
{
