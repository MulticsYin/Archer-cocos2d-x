...
Animation* PlaySceneOne::createAnimation(std::string prefixName, int framesNum, float delay)
{
...
void PlaySceneOne::onEnter()
{
	...
	//加入设置背景图片；
	auto gameSprite = Sprite::create("map/bj1.jpg");
	...
	contactListener->onContactBegin = CC_CALLBACK_1(PlaySceneOne::onContactBegin, this);
	contactListener->onContactPostSolve = CC_CALLBACK_2(PlaySceneOne::onContactPostSolve, this);
	contactListener->onContactPreSolve = CC_CALLBACK_2(PlaySceneOne::onContactPreSolve, this);
	contactListener->onContactSeparate = CC_CALLBACK_1(PlaySceneOne::onContactSeparate, this);
	...
	auto pauseMenuItem = MenuItemSprite::create(pauseSprite, pauseSprite,
		CC_CALLBACK_1(PlaySceneOne::menuPauseCallback, this));
	...
	hero->setPosition(map->print_x , map->print_y);
	...
	//加入测试用的关卡号
	auto star = Sprite::create("scene1.png");	//静态的
	star->setAnchorPoint(Point(0, 0));
	star->setPosition(visibleSize.width * 9 / 10, visibleSize.height * 9 / 10);
	this->addChild(star, 200);

	//Update每帧执行一次（1/60秒）
	schedule(schedule_selector(PlaySceneOne::Update));
}

bool PlaySceneOne::onTouchBegan(Touch* touches, Event* event) 
{
	log("touchBeganNNNNN执行了---------------------------------------------");
...
void PlaySceneOne::onTouchEnded(Touch* touches, Event* event) 
{
	Touch* touch = static_cast<Touch*>(touches);
	Point locationInNodelocal = touch->getLocation();
	ontouch = 0;
...
bool PlaySceneOne::onContactBegin(PhysicsContact &contact) {
	...
	{
		map->dead->removeFromParentAndCleanup(true);
	}
	break;
	case ARROW_BIT_MASK|GROUND_BIT_MASK://箭射中店面
	{
		//hero->arrow->removeFromParentAndCleanup(true);
...
bool PlaySceneOne::onContactPreSolve(PhysicsContact& contact, PhysicsContactPreSolve& solve) {
	return true;
}

void PlaySceneOne::onContactPostSolve(PhysicsContact& contact, const PhysicsContactPostSolve& solve) {

}

void PlaySceneOne::onContactSeparate(PhysicsContact& contact) {

}

// 计时器更新函数，每帧一次
void PlaySceneOne::Update(float time) {
	log("UPDATE             !!!!!!!!!!!!");
...
/*******		控制屏幕上下移动   *******************
	else if (hero->getPosition().y>visibleSize.height * 2 / 3)
	{
		auto offset = hero->getPosition().y - visibleSize.height * 2 / 3;
		//hero->stopAllActions();
		map->setPosition(Vec2(map->getPosition().x, map->getPosition().y - offset));
		hero->setPosition(Vec2(hero->getPosition().x, visibleSize.height * 2 / 3 - 1));
	}
	else if (hero->getPosition().y<visibleSize.height / 3)
	{
		auto offset = visibleSize.height / 3 - hero->getPosition().y;
		//hero->stopAllActions();
		map->setPosition(Vec2(map->getPosition().x, map->getPosition().y + offset));
		hero->setPosition(Vec2(hero->getPosition().x, visibleSize.height / 3 + 1));
	}
*/
...
void PlaySceneOne::onExit() {
	Layer::onExit();
	//停止调用 shootBullet函数.
	this->unschedule(schedule_selector(PlaySceneOne::Update));
	//注销事件监听器.
...
void PlaySceneOne::menuPauseCallback(Ref* pSender) {	//暂停按钮
	...
	auto backMenuItem = MenuItemSprite::create(backNormal, backSelected,
		CC_CALLBACK_1(PlaySceneOne::menuBackCallback, this));
	...
	auto resumeMenuItem = MenuItemSprite::create(resumeNormal, resumeSelected,
		CC_CALLBACK_1(PlaySceneOne::menuResumeCallback, this));
...
void PlaySceneOne::menuBackCallback(Ref* pSender)	//回到主界面
{
...
void PlaySceneOne::menuResumeCallback(Ref* pSender)		//回到游戏
{
