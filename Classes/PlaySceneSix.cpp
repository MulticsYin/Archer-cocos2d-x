...
Animation* PlaySceneSix::createAnimation(std::string prefixName, int framesNum, float delay)
{
...
void PlaySceneSix::onEnter()
{
	...
	contactListener->onContactBegin = CC_CALLBACK_1(PlaySceneSix::onContactBegin, this);
	contactListener->onContactPostSolve = CC_CALLBACK_2(PlaySceneSix::onContactPostSolve, this);
	contactListener->onContactPreSolve = CC_CALLBACK_2(PlaySceneSix::onContactPreSolve, this);
	contactListener->onContactSeparate = CC_CALLBACK_1(PlaySceneSix::onContactSeparate, this);
	...
	auto pauseMenuItem = MenuItemSprite::create(pauseSprite, pauseSprite,
		CC_CALLBACK_1(PlaySceneSix::menuPauseCallback, this));
	...
	//加入测试用的关卡号
	auto star = Sprite::create("scene6.png");	//静态的
	star->setAnchorPoint(Point(0, 0));
	star->setPosition(visibleSize.width * 9 / 10, visibleSize.height * 9 / 10);
	this->addChild(star, 200);

	//Update每帧执行一次（1/60秒）
	schedule(schedule_selector(PlaySceneSix::Update));
}

bool PlaySceneSix::onTouchBegan(Touch* touches, Event* event)
{
	log("touchBeganNNNNN执行了---------------------------------------------");
...
void PlaySceneSix::onTouchMoved(Touch* touches, Event* event)
{
	log("touchMMMMMMMMMOOVVEDDD执行了---------------------------------------------");
	// 获取当前触摸的目标
	Touch* touch = static_cast<Touch*>(touches);
...
void PlaySceneSix::onTouchEnded(Touch* touches, Event* event)
{
	Touch* touch = static_cast<Touch*>(touches);
	Point locationInNodelocal = touch->getLocation();
	ontouch = 0;
...
bool PlaySceneSix::onContactBegin(PhysicsContact &contact) {
...
bool PlaySceneSix::onContactPreSolve(PhysicsContact& contact, PhysicsContactPreSolve& solve) {
	return true;
}

void PlaySceneSix::onContactPostSolve(PhysicsContact& contact, const PhysicsContactPostSolve& solve) {

}

void PlaySceneSix::onContactSeparate(PhysicsContact& contact) {

}

// 计时器更新函数，每帧一次
void PlaySceneSix::Update(float time) {
	log("UPDATE             !!!!!!!!!!!!");
...
void PlaySceneSix::onExit() {
	Layer::onExit();
	//停止调用 shootBullet函数.
	this->unschedule(schedule_selector(PlaySceneSix::Update));
	//注销事件监听器.
...
void PlaySceneSix::onEnterTransitionDidFinish()
{
...
void PlaySceneSix::menuPauseCallback(Ref* pSender) {	//暂停按钮
	...
	auto backMenuItem = MenuItemSprite::create(backNormal, backSelected,
		CC_CALLBACK_1(PlaySceneSix::menuBackCallback, this));
	...
	auto resumeMenuItem = MenuItemSprite::create(resumeNormal, resumeSelected,
		CC_CALLBACK_1(PlaySceneSix::menuResumeCallback, this));
...
void PlaySceneSix::menuBackCallback(Ref* pSender)	//回到主界面
{
...
void PlaySceneSix::menuResumeCallback(Ref* pSender)		//回到游戏
{
