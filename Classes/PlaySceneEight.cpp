...
Animation* PlaySceneEight::createAnimation(std::string prefixName, int framesNum, float delay)
{
...
void PlaySceneEight::onEnter()
{
	...
	contactListener->onContactBegin = CC_CALLBACK_1(PlaySceneEight::onContactBegin, this);
	contactListener->onContactPostSolve = CC_CALLBACK_2(PlaySceneEight::onContactPostSolve, this);
	contactListener->onContactPreSolve = CC_CALLBACK_2(PlaySceneEight::onContactPreSolve, this);
	contactListener->onContactSeparate = CC_CALLBACK_1(PlaySceneEight::onContactSeparate, this);
	...
	auto pauseMenuItem = MenuItemSprite::create(pauseSprite, pauseSprite,
		CC_CALLBACK_1(PlaySceneEight::menuPauseCallback, this));
	...
	//Update每帧执行一次（1/60秒）
	schedule(schedule_selector(PlaySceneEight::Update));
}

bool PlaySceneEight::onTouchBegan(Touch* touches, Event* event)
{
	log("touchBeganNNNNN执行了---------------------------------------------");
...
void PlaySceneEight::onTouchMoved(Touch* touches, Event* event)
{
	log("touchMMMMMMMMMOOVVEDDD执行了---------------------------------------------");
	// 获取当前触摸的目标
	Touch* touch = static_cast<Touch*>(touches);
...
void PlaySceneEight::onTouchEnded(Touch* touches, Event* event)
{
	Touch* touch = static_cast<Touch*>(touches);
	Point locationInNodelocal = touch->getLocation();
	ontouch = 0;
...
bool PlaySceneEight::onContactBegin(PhysicsContact &contact) {
...
bool PlaySceneEight::onContactPreSolve(PhysicsContact& contact, PhysicsContactPreSolve& solve) {
	return true;
}

void PlaySceneEight::onContactPostSolve(PhysicsContact& contact, const PhysicsContactPostSolve& solve) {

}

void PlaySceneEight::onContactSeparate(PhysicsContact& contact) {

}

// 计时器更新函数，每帧一次
void PlaySceneEight::Update(float time) {
	log("UPDATE             !!!!!!!!!!!!");
...
void PlaySceneEight::onExit() {
	Layer::onExit();
	//停止调用 shootBullet函数.
	this->unschedule(schedule_selector(PlaySceneEight::Update));
	//注销事件监听器.
...
void PlaySceneEight::onEnterTransitionDidFinish()
{
...
void PlaySceneEight::isGameOver()
{
	//无法返回游戏界面
		//选择重来
		log("GameOver");
...
void PlaySceneEight::menuPauseCallback(Ref* pSender) {	//暂停按钮
	...
	auto backMenuItem = MenuItemSprite::create(backNormal, backSelected,
		CC_CALLBACK_1(PlaySceneEight::menuBackCallback, this));
	...
	auto resumeMenuItem = MenuItemSprite::create(resumeNormal, resumeSelected,
		CC_CALLBACK_1(PlaySceneEight::menuResumeCallback, this));
...
void PlaySceneEight::menuBackCallback(Ref* pSender)	//回到主界面
{
...
void PlaySceneEight::menuResumeCallback(Ref* pSender)		//回到游戏
{
