...
PlayMapSeven::~PlayMapSeven() {

}

void PlayMapSeven::onEnter() {
	Layer::onEnter();
	...
	contactListener->onContactBegin = CC_CALLBACK_1(PlayMapSeven::onContactBegin, this);
	contactListener->onContactPostSolve = CC_CALLBACK_2(PlayMapSeven::onContactPostSolve, this);
	contactListener->onContactPreSolve = CC_CALLBACK_2(PlayMapSeven::onContactPreSolve, this);
	contactListener->onContactSeparate = CC_CALLBACK_1(PlayMapSeven::onContactSeparate, this);
	//事件分发器
	...
	//map00瓦片地图地图
	tilemap = TMXTiledMap::create("map/map7.tmx");
...
/*	for (int y = 0; y<TMX_HEIGHT; y++)
	{
		...
		for (int x = 0; x<TMX_WIDTH; x++)
		{
			...
			auto gid = layer->getTileGIDAt(Vec2(x, y));//为了提高点效率，我们没必要给每个tile加上刚体，在创建地图时我们设定了空白处的gid值为12，因此我们只对非12的tile加上刚体
			log("GGIIDD %d",gid);
			switch (gid)
			{
			case GROUND_GID:
...
void PlayMapSeven::update(float time) {

}

void PlayMapSeven::onExit() {
	Layer::onExit();
...
bool PlayMapSeven::onContactBegin(PhysicsContact &contact) {
...
bool PlayMapSeven::onContactPreSolve(PhysicsContact& contact, PhysicsContactPreSolve& solve) {
	return true;
}

void PlayMapSeven::onContactPostSolve(PhysicsContact& contact, const PhysicsContactPostSolve& solve) {

}

void PlayMapSeven::onContactSeparate(PhysicsContact& contact) {

}

void PlayMapSeven::makePhysicsObjAt(Point p, Size size, bool d, float r, float f, float dens, float rest,int gid,int tag)
{
