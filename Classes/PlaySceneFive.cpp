...
Animation* PlaySceneFive::createAnimation(std::string prefixName, int framesNum, float delay)
{
...
void PlaySceneFive::onEnter()
{
	...
	contactListener->onContactBegin = CC_CALLBACK_1(PlaySceneFive::onContactBegin, this);
	contactListener->onContactPostSolve = CC_CALLBACK_2(PlaySceneFive::onContactPostSolve, this);
	contactListener->onContactPreSolve = CC_CALLBACK_2(PlaySceneFive::onContactPreSolve, this);
	contactListener->onContactSeparate = CC_CALLBACK_1(PlaySceneFive::onContactSeparate, this);
	...
	auto pauseMenuItem = MenuItemSprite::create(pauseSprite, pauseSprite,
		CC_CALLBACK_1(PlaySceneFive::menuPauseCallback, this));
	...
	//加入测试用的关卡号
	auto star = Sprite::create("scene5.png");	//静态的
	star->setAnchorPoint(Point(0, 0));
	star->setPosition(visibleSize.width * 9 / 10, visibleSize.height * 9 / 10);
	this->addChild(star, 200);

	//Update每帧执行一次（1/60秒）
	schedule(schedule_selector(PlaySceneFive::Update));
}

bool PlaySceneFive::onTouchBegan(Touch* touches, Event* event)
{
	log("touchBeganNNNNN执行了---------------------------------------------");
...
void PlaySceneFive::onTouchMoved(Touch* touches, Event* event)
{
	log("touchMMMMMMMMMOOVVEDDD执行了---------------------------------------------");
	// 获取当前触摸的目标
	Touch* touch = static_cast<Touch*>(touches);
...
void PlaySceneFive::onTouchEnded(Touch* touches, Event* event)
{
	Touch* touch = static_cast<Touch*>(touches);
	Point locationInNodelocal = touch->getLocation();
	ontouch = 0;
...
bool PlaySceneFive::onContactBegin(PhysicsContact &contact) {
...
bool PlaySceneFive::onContactPreSolve(PhysicsContact& contact, PhysicsContactPreSolve& solve) {
	return true;
}

void PlaySceneFive::onContactPostSolve(PhysicsContact& contact, const PhysicsContactPostSolve& solve) {

}

void PlaySceneFive::onContactSeparate(PhysicsContact& contact) {

}

// 计时器更新函数，每帧一次
void PlaySceneFive::Update(float time) {
	log("UPDATE             !!!!!!!!!!!!");
...
void PlaySceneFive::onExit() {
	Layer::onExit();
	//停止调用 shootBullet函数.
	this->unschedule(schedule_selector(PlaySceneFive::Update));
	//注销事件监听器.
...
void PlaySceneFive::onEnterTransitionDidFinish()
{
...
void PlaySceneFive::menuPauseCallback(Ref* pSender) {	//暂停按钮
	...
	auto backMenuItem = MenuItemSprite::create(backNormal, backSelected,
		CC_CALLBACK_1(PlaySceneFive::menuBackCallback, this));
	...
	auto resumeMenuItem = MenuItemSprite::create(resumeNormal, resumeSelected,
		CC_CALLBACK_1(PlaySceneFive::menuResumeCallback, this));
...
void PlaySceneFive::menuBackCallback(Ref* pSender)	//回到主界面
{
...
void PlaySceneFive::menuResumeCallback(Ref* pSender)		//回到游戏
{
