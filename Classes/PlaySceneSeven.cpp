...
Animation* PlaySceneSeven::createAnimation(std::string prefixName, int framesNum, float delay)
{
...
void PlaySceneSeven::onEnter()
{
	...
	contactListener->onContactBegin = CC_CALLBACK_1(PlaySceneSeven::onContactBegin, this);
	contactListener->onContactPostSolve = CC_CALLBACK_2(PlaySceneSeven::onContactPostSolve, this);
	contactListener->onContactPreSolve = CC_CALLBACK_2(PlaySceneSeven::onContactPreSolve, this);
	contactListener->onContactSeparate = CC_CALLBACK_1(PlaySceneSeven::onContactSeparate, this);
	...
	auto pauseMenuItem = MenuItemSprite::create(pauseSprite, pauseSprite,
		CC_CALLBACK_1(PlaySceneSeven::menuPauseCallback, this));
	...
	//加入测试用的关卡号
	auto star = Sprite::create("scene7.png");	//静态的
	star->setAnchorPoint(Point(0, 0));
	star->setPosition(visibleSize.width * 9 / 10, visibleSize.height * 9 / 10);
	this->addChild(star, 200);

	//Update每帧执行一次（1/60秒）
	schedule(schedule_selector(PlaySceneSeven::Update));
}

bool PlaySceneSeven::onTouchBegan(Touch* touches, Event* event)
{
	log("touchBeganNNNNN执行了---------------------------------------------");
...
void PlaySceneSeven::onTouchMoved(Touch* touches, Event* event)
{
	log("touchMMMMMMMMMOOVVEDDD执行了---------------------------------------------");
	// 获取当前触摸的目标
	Touch* touch = static_cast<Touch*>(touches);
...
void PlaySceneSeven::onTouchEnded(Touch* touches, Event* event)
{
	Touch* touch = static_cast<Touch*>(touches);
	Point locationInNodelocal = touch->getLocation();
	ontouch = 0;
...
bool PlaySceneSeven::onContactBegin(PhysicsContact &contact) {
...
bool PlaySceneSeven::onContactPreSolve(PhysicsContact& contact, PhysicsContactPreSolve& solve) {
	return true;
}

void PlaySceneSeven::onContactPostSolve(PhysicsContact& contact, const PhysicsContactPostSolve& solve) {

}

void PlaySceneSeven::onContactSeparate(PhysicsContact& contact) {

}

// 计时器更新函数，每帧一次
void PlaySceneSeven::Update(float time) {
	log("UPDATE             !!!!!!!!!!!!");
...
void PlaySceneSeven::onExit() {
	Layer::onExit();
	//停止调用 shootBullet函数.
	this->unschedule(schedule_selector(PlaySceneSeven::Update));
	//注销事件监听器.
...
void PlaySceneSeven::onEnterTransitionDidFinish()
{
...
void PlaySceneSeven::isGameOver()
{
	//无法返回游戏界面
		//选择重来
		log("GameOver");
...
void PlaySceneSeven::menuPauseCallback(Ref* pSender) {	//暂停按钮
	...
	auto backMenuItem = MenuItemSprite::create(backNormal, backSelected,
		CC_CALLBACK_1(PlaySceneSeven::menuBackCallback, this));
	...
	auto resumeMenuItem = MenuItemSprite::create(resumeNormal, resumeSelected,
		CC_CALLBACK_1(PlaySceneSeven::menuResumeCallback, this));
...
void PlaySceneSeven::menuBackCallback(Ref* pSender)	//回到主界面
{
...
void PlaySceneSeven::menuResumeCallback(Ref* pSender)		//回到游戏
{
