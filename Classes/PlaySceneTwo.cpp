...
Animation* PlaySceneTwo::createAnimation(std::string prefixName, int framesNum, float delay)
{
...
void PlaySceneTwo::onEnter()
{
	...
	contactListener->onContactBegin = CC_CALLBACK_1(PlaySceneTwo::onContactBegin, this);
	contactListener->onContactPostSolve = CC_CALLBACK_2(PlaySceneTwo::onContactPostSolve, this);
	contactListener->onContactPreSolve = CC_CALLBACK_2(PlaySceneTwo::onContactPreSolve, this);
	contactListener->onContactSeparate = CC_CALLBACK_1(PlaySceneTwo::onContactSeparate, this);
	...
	auto pauseMenuItem = MenuItemSprite::create(pauseSprite, pauseSprite,
		CC_CALLBACK_1(PlaySceneTwo::menuPauseCallback, this));
	...
	//加入测试用的关卡号
	auto star = Sprite::create("scene2.png");	//静态的
	star->setAnchorPoint(Point(0, 0));
	star->setPosition(visibleSize.width * 9 / 10, visibleSize.height * 9 / 10);
	this->addChild(star, 200);

	//Update每帧执行一次（1/60秒）
	schedule(schedule_selector(PlaySceneTwo::Update));
}

bool PlaySceneTwo::onTouchBegan(Touch* touches, Event* event)
{
	log("touchBeganNNNNN执行了---------------------------------------------");
...
void PlaySceneTwo::onTouchMoved(Touch* touches, Event* event)
{
	log("touchMMMMMMMMMOOVVEDDD执行了---------------------------------------------");
	// 获取当前触摸的目标
	Touch* touch = static_cast<Touch*>(touches);
...
void PlaySceneTwo::onTouchEnded(Touch* touches, Event* event)
{
	Touch* touch = static_cast<Touch*>(touches);
	Point locationInNodelocal = touch->getLocation();
	ontouch = 0;
...
bool PlaySceneTwo::onContactBegin(PhysicsContact &contact) {
...
bool PlaySceneTwo::onContactPreSolve(PhysicsContact& contact, PhysicsContactPreSolve& solve) {
	return true;
}

void PlaySceneTwo::onContactPostSolve(PhysicsContact& contact, const PhysicsContactPostSolve& solve) {

}

void PlaySceneTwo::onContactSeparate(PhysicsContact& contact) {

}

// 计时器更新函数，每帧一次
void PlaySceneTwo::Update(float time) {
	log("UPDATE             !!!!!!!!!!!!");
...
void PlaySceneTwo::onExit() {
	Layer::onExit();
	//停止调用 shootBullet函数.
	this->unschedule(schedule_selector(PlaySceneTwo::Update));
	//注销事件监听器.
...
void PlaySceneTwo::onEnterTransitionDidFinish()
{
...
void PlaySceneTwo::menuPauseCallback(Ref* pSender) {	//暂停按钮
	...
	auto backMenuItem = MenuItemSprite::create(backNormal, backSelected,
		CC_CALLBACK_1(PlaySceneTwo::menuBackCallback, this));
	...
	auto resumeMenuItem = MenuItemSprite::create(resumeNormal, resumeSelected,
		CC_CALLBACK_1(PlaySceneTwo::menuResumeCallback, this));
...
void PlaySceneTwo::menuBackCallback(Ref* pSender)	//回到主界面
{
...
void PlaySceneTwo::menuResumeCallback(Ref* pSender)		//回到游戏
{
