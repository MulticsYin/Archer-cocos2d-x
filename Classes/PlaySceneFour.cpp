...
Animation* PlaySceneFour::createAnimation(std::string prefixName, int framesNum, float delay)
{
...
void PlaySceneFour::onEnter()
{
	...
	contactListener->onContactBegin = CC_CALLBACK_1(PlaySceneFour::onContactBegin, this);
	contactListener->onContactPostSolve = CC_CALLBACK_2(PlaySceneFour::onContactPostSolve, this);
	contactListener->onContactPreSolve = CC_CALLBACK_2(PlaySceneFour::onContactPreSolve, this);
	contactListener->onContactSeparate = CC_CALLBACK_1(PlaySceneFour::onContactSeparate, this);
	...
	auto pauseMenuItem = MenuItemSprite::create(pauseSprite, pauseSprite,
		CC_CALLBACK_1(PlaySceneFour::menuPauseCallback, this));
	...
	//加入测试用的关卡号
	auto star = Sprite::create("scene4.png");	//静态的
	star->setAnchorPoint(Point(0, 0));
	star->setPosition(visibleSize.width * 9 / 10, visibleSize.height * 9 / 10);
	this->addChild(star, 200);

	//Update每帧执行一次（1/60秒）
	schedule(schedule_selector(PlaySceneFour::Update));
}

bool PlaySceneFour::onTouchBegan(Touch* touches, Event* event)
{
	log("touchBeganNNNNN执行了---------------------------------------------");
...
void PlaySceneFour::onTouchMoved(Touch* touches, Event* event)
{
	log("touchMMMMMMMMMOOVVEDDD执行了---------------------------------------------");
	// 获取当前触摸的目标
	Touch* touch = static_cast<Touch*>(touches);
...
void PlaySceneFour::onTouchEnded(Touch* touches, Event* event)
{
	Touch* touch = static_cast<Touch*>(touches);
	Point locationInNodelocal = touch->getLocation();
	ontouch = 0;
...
bool PlaySceneFour::onContactBegin(PhysicsContact &contact) {
...
bool PlaySceneFour::onContactPreSolve(PhysicsContact& contact, PhysicsContactPreSolve& solve) {
	return true;
}

void PlaySceneFour::onContactPostSolve(PhysicsContact& contact, const PhysicsContactPostSolve& solve) {

}

void PlaySceneFour::onContactSeparate(PhysicsContact& contact) {

}

// 计时器更新函数，每帧一次
void PlaySceneFour::Update(float time) {
	log("UPDATE             !!!!!!!!!!!!");
...
void PlaySceneFour::onExit() {
	Layer::onExit();
	//停止调用 shootBullet函数.
	this->unschedule(schedule_selector(PlaySceneFour::Update));
	//注销事件监听器.
...
void PlaySceneFour::onEnterTransitionDidFinish()
{
...
void PlaySceneFour::menuPauseCallback(Ref* pSender) {	//暂停按钮
	...
	auto backMenuItem = MenuItemSprite::create(backNormal, backSelected,
		CC_CALLBACK_1(PlaySceneFour::menuBackCallback, this));
	...
	auto resumeMenuItem = MenuItemSprite::create(resumeNormal, resumeSelected,
		CC_CALLBACK_1(PlaySceneFour::menuResumeCallback, this));
...
void PlaySceneFour::menuBackCallback(Ref* pSender)	//回到主界面
{
...
void PlaySceneFour::menuResumeCallback(Ref* pSender)		//回到游戏
{
